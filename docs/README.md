# JOOservices Laravel Notifications Documentation

Documentation for `jooservices/laravel-notifications` (`JOOservices\LaravelNotifications\`). Current line: **v1.0.1**.

## Recommended reading order

1. [Installation](./01-getting-started/installation.md)
2. [Quick Start](./01-getting-started/quick-start.md)
3. [Channels](./02-user-guide/channels.md)
4. [Configuration](./02-user-guide/configuration.md)
5. [Send results](./02-user-guide/send-results.md)

## Documentation map

### Architecture

- [Project Overview](./00-architecture/01-project-overview.md)
- [Repository Structure](./00-architecture/02-repository-structure.md)

### Getting started

- [Installation](./01-getting-started/installation.md)
- [Quick Start](./01-getting-started/quick-start.md)

### User guide

- [Channels](./02-user-guide/channels.md)
- [Configuration](./02-user-guide/configuration.md)
- [Send results](./02-user-guide/send-results.md)

### Examples

- [Application usage](./03-examples/application-usage.md)

### Maintenance

- [Release checklist](./05-maintenance/03-release-checklist.md)
